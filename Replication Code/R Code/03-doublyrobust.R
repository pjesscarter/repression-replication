...
#Note - cross-validation does not work well due to small sample (
#extremely sensitive to selection and tends to cause collinearit)
...
#Use superleaner for OR and average over multiple calls
drobust_reg <- drtmle(W = fnl_reg[,controls], A = fnl_reg$D, Y = fnl_reg$Share_reg70_w2,
                  SL_g = SL_library, SL_Q = SL_library,
                  SL_gr = SL_library, SL_Qr = SL_library,
                  cvFolds=2,
                  avg_over= c("drtmle", "SL"),
